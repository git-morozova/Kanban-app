...
export const usersActivator = function () {
  ...
  //клик по кнопке add
  document.querySelector('#app-users-submit').addEventListener('click', function (event) {
    ...
    document.querySelector('#app-inputs-delete').addEventListener('click', function (event) {
      event.preventDefault();
      event.stopImmediatePropagation();      
      ...
      for (let i = 0; i < usersStorage.length; i++) {
        ...
        <div class="select-box__value">
          ...
          <p class="select-box__input-text" id="p-${i+1}">${usersStorage[i].login}</p>
        </div>`;
        listInputs.insertAdjacentHTML('beforeend', listInput);

        let listLabel = `
        <li>
          <label class="select-box__option" for="${id}" aria-hidden="aria-hidden">${usersStorage[i].login}</label>
        </li>`;
        listLabels.insertAdjacentHTML('beforeend', listLabel);
      };

      //клик по кнопке Delete
      document.querySelector('#app-users-delete').addEventListener('click', function (event) { 
        event.preventDefault();
        event.stopImmediatePropagation();
        ...
        for (let i = 0; i < usersStorage.length; i++) {
          ...
          pseudo.getPropertyValue("display") == "block" ? login = option.innerHTML : false;
        };
        if(login == "") return;
      ...
      const clearSelectNodes = function () {
        let listLabels = document.querySelector('#app-users-delete');
